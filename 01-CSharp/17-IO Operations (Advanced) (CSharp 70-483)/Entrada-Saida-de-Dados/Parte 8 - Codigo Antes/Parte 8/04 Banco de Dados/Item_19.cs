...
namespace Listings
{
    class Item_19 //Ler Dados com SQL
    {
        ...
        private const string CONNECTION_STRING = @"Data Source=(localdb)\MSSQLLocalDB;Initial Catalog=Cinema;Integrated Security=True;Connect Timeout=30;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False";
        ...
        static async Task XMain(string[] args)
        {
           //await CriarBancoDeDadosAsync();
            ...
            using (var conexao = new SqlConnection(CONNECTION_STRING))
            {
                ...
                using (var query = new SqlCommand(@"SELECT Nome AS Diretor, Titulo, Minutos FROM Filmes AS d
                                           INNER JOIN Diretores ON d.DiretorId = Diretores.Id", conexao))
                {
                    SqlDataReader leitor = await query.ExecuteReaderAsync();
                    while(await leitor.ReadAsync())
                    {
                        var diretor = leitor["Diretor"];
                        var filme = leitor["Titulo"];
                        Console.Write($"{diretor}\t{filme}\n");
                    }
