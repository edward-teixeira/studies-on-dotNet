...
namespace Program01
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var consulta2 = from f in filmes.AsParallel()
                            where f.Genero == "Adventure"
                            select f;
            ...
            var consulta3 = from f in filmes
                            ...
                            where f.Genero == "Adventure"
                            select f;
            //GeraRelatorio("Tarefa 3: obter a lista de filmes de Aventura, executando em PARALELO com modo de execução default", consulta2);
            ...
            var consulta6 = from f in filmes.AsParallel()
                                            .AsOrdered()
                            where f.Genero == "Adventure"
                            group f by f.Faturamento into faturamentoGroup
                            orderby faturamentoGroup.Key descending
                            select faturamentoGroup;
            

            //foreach (var faturamentoGroup in consulta6.Take(5))
            //{
            //    foreach (var filme in faturamentoGroup)
            //    {
            //        Console.WriteLine($"{filme.Titulo}\t{filme.Genero}\t{filme.Faturamento}");
            //    }
                
            //}

            //GeraRelatorio("Tarefa 6: obter a lista de filmes de Aventura, executando em PARALELO forçando paralelismo e com grau de paralelismo = 4", consulta6);
